...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.integer('id_experto').unsigned().references('id_usuario').inTable('usuarios').onDelete('CASCADE').notNullable()
      table.integer('id_finca').unsigned().references('id_finca').inTable('fincas').onDelete('CASCADE').notNullable()
