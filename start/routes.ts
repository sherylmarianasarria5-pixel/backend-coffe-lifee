...
const CatTiposTratamientosController = () =>
  import('#controllers/cat_tipos_tratamientos_controller')
...
const CatTiposRecomendacionesController = () =>
  import('#controllers/cat_tipos_recomendaciones_controller')
...
const AplicacionesTratamientosController = () =>
  import('#controllers/aplicaciones_tratamientos_controller')
...
router
  .post('/test-notificacion', async ({ request, response }) => {
...
// Quitar cuando ya no se necesite.
router
  .post('/test-push', async ({ request, response }) => {
    const jwt = (request as any).usuarioJwt
    if (!jwt?.id) return response.unauthorized({ message: 'Token requerido' })
